...
### Создание SUPER_ADMIN через одноразовую ссылку

Для добавления владельца без передачи временного пароля доступны разовые startup-настройки:

- `PROVISION_SUPER_ADMIN=1`;
- `PROVISION_ADMIN_LOGIN`;
- `PROVISION_ADMIN_DISPLAY_NAME`;
- secret `PROVISION_ADMIN_RESET_TOKEN` — случайный base64url-токен из 32 байт.

После старта приложение создаёт только активного `SUPER_ADMIN` с указанным свободным логином либо выпускает новую ссылку для уже существующего активного `SUPER_ADMIN`. Начальный пароль нового аккаунта случаен и неизвестен. Токен хранится только как SHA-256-хеш, действует два часа и передаётся владельцу в URL-фрагменте:

```text
https://<public-amvera-domain>/reset-password#token=<token>
```

Фрагмент после `#` не отправляется серверу при открытии страницы и не попадает в стандартный access log. После установки пароля токен помечается использованным, а все старые сессии пользователя отзываются. Сразу после сообщения об успешном provisioning удалить четыре временные настройки и перезапустить приложение. Использованный токен повторно не активируется.
