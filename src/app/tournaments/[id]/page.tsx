...
export default async function TournamentPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ error?: string }> }) {
  ...
  return <main className="page tournament-page">
    <section className="page-hero tournament-hero"><div><p className="eyebrow">Миникап · 5 туров{tournament.archivedAt ? " · Архив" : ""}</p><h1>{tournament.name}</h1><p className="lead">{final ? "Миникап завершён" : completedRounds ? `${completedRounds} из 5 игр завершено` : "Всё готово к первому туру"}</p><p className="assigned-judges">Судьи: {tournament.judges.length ? tournament.judges.map(({ user: judge }) => judge.displayName).join(", ") : "не назначены"}</p></div><div className="tournament-score"><b>{completedRounds}</b><span>из 5 игр</span></div></section>
    <ol className="round-progress" aria-label="Прогресс турнира">{tournament.rounds.map((round) => <li className={round.status === "COMPLETED" ? "done" : availableRound?.id === round.id ? "current" : ""} key={round.id}><span>{round.status === "COMPLETED" ? "✓" : round.number}</span><small>Тур {round.number}</small></li>)}</ol>
