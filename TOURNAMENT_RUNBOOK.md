...
- войти под личной учётной записью и проверить, что судья назначен на нужный турнир;
...
Если приложение вернуло экран входа, войти повторно. Не передавать пароль другого судьи и не создавать общий аккаунт. Если доступ к турниру отсутствует, HEAD_JUDGE/SUPER_ADMIN должен проверить назначение судьи на карточке турнира.

## Если потерян доступ

- SUPER_ADMIN активирует пользователя или сбрасывает пароль в `/admin/users`;
- после деактивации/сброса старые сессии автоматически недействительны;
- не изменять пользователей напрямую в PostgreSQL во время турнира;
- если активных SUPER_ADMIN не осталось, остановить административные изменения и использовать CLI-процедуру из `DEPLOYMENT.md`.
